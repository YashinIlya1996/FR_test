...
class Message(models.Model):
    ...
    send_date_time = models.DateTimeField(null=True)
    status = models.IntegerField(default=PLANNED, choices=STATUS_CHOICES)
    mailing = models.ForeignKey(Mailing, on_delete=models.CASCADE, related_name='messages',
                                related_query_name='q_messages')
    client = models.ForeignKey(Client, on_delete=models.CASCADE, related_name='clients')

    def __str__(self):
        return f"Status: {self.get_status_display()}, to {self.client.phone_number}, " \
